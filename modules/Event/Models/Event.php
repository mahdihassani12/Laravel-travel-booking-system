...
class Event extends Bookable
{
    ...
    {
        $model_event = parent::query()->select("bc_events.*");
        $model_event->where("bc_events.status", "publish");
        if (!empty($location_id = $request->query('location_id'))) {
            $location = Location::where('id', $location_id)->where("status","publish")->first();
            if(!empty($location)){
                $model_event->join('bc_locations', function ($join) use ($location) {
                    $join->on('bc_locations.id', '=', 'bc_events.location_id')
        ...
        if (is_array($terms) && !empty($terms)) {
            $model_event->join('bc_event_term as tt', 'tt.target_id', "bc_events.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bc_events.review_score >= ? AND bc_events.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_event->leftJoin('bc_event_translations', function ($join) {
                    $join->on('bc_events.id', '=', 'bc_event_translations.origin_id');
                });
                $model_event->where('bc_event_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_event->where('bc_events.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $model_event->orderByRaw("POW((bc_events.map_lng-?),2) + POW((bc_events.map_lat-?),2)",[$lgn,$lat]);
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bc_events.sale_price, 0 ) > 0 THEN bc_events.sale_price ELSE bc_events.price END AS tmp_min_price";
        ...
        $model_event->groupBy("bc_events.id");
