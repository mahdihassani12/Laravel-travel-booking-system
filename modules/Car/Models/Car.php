...
class Car extends Bookable
{
    ...
    {
        $model_car = parent::query()->select("bc_cars.*");
        ...
        if (is_array($terms) && !empty($terms)) {
            $model_car->join('bc_car_term as tt', 'tt.target_id', "bc_cars.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bc_cars.review_score >= ? AND bc_cars.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
        ...
        if(!empty( $service_name = $request->query("service_name") )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_car->leftJoin('bc_car_translations', function ($join) {
                    $join->on('bc_cars.id', '=', 'bc_car_translations.origin_id');
                });
                $model_car->where('bc_car_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_car->where('bc_cars.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))){
            $model_car->orderByRaw("POW((bc_cars.map_lng-".$lgn."),2) + POW((bc_cars.map_lat-".$lat."),2)");
        }
        $orderby = $request->input("orderby");
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bc_cars.sale_price, 0 ) > 0 THEN bc_cars.sale_price ELSE bc_cars.price END AS tmp_min_price";
